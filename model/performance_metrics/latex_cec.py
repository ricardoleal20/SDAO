"""
Generate LaTeX tables for CEC (scenario 3, d=50).

Outputs:
1) winners_cec.tex: Best mean absolute error per function and the winning algorithm.
2) cec_convergence_iterations.tex: Average iteration of numerical stability (<1e-4) per algorithm.
3) cec_mean_std.tex: Mean ± std (absolute error) per algorithm and function.

Input: scenario_3_d_50.pkl in the current working directory.
"""
...
def _load_data() -> BENCH_DATA:
    p = Path.cwd() / "scenario_3_d_50.pkl"
    if not p.exists():
        raise FileNotFoundError(
            "Expected scenario_3_d_50.pkl in the current directory."
        )
    with p.open("rb") as f:
        data = pickle.load(f)
...
def _get_all_functions() -> List[str]:
    return [f["name"] for f in cec_funcs]
...
def generate_winners_table(data: BENCH_DATA) -> None:
    ...
    _write_file(Path("winners_cec.tex"), content)


def generate_convergence_table(data: BENCH_DATA) -> None:
    conv = convergence_rate(data, scenario=3)
    algorithms = _get_all_algorithms(data)
    functions = _get_all_functions()
    ...
    _write_file(Path("cec_convergence_iterations.tex"), content)
...
def generate_mean_std_table(data: BENCH_DATA) -> None:
    ...
    _write_file(Path("cec_mean_std.tex"), content)
