...
class SHADEwithILS(Algorithm):  # pylint: disable=R0903
    ...
    __slots__ = [
        ...
        "_memory_size", "_memory_cr", "_memory_f",
        "_verbose"
    ]
    ...
    def optimize(  # pylint: disable=R0914
        ...
        for iteration in range(self._n_iterations):
            ...
            for i in range(self._n_population):
                ...
                cr_pop[i] = np.clip(np.random.normal(
                    self._memory_cr[idx], 0.1), 0, 1)
                f_pop[i] = np.clip(np.random.normal(
                    self._memory_f[idx], 0.1), 0, 1)

                # Mutation and Crossover
                r1, r2, r3 = np.random.choice(
                    [j for j in range(self._n_population) if j != i], 3, replace=False
                )
                mutant = population[r1] + f_pop[i] * \
                ...
                trial = np.where(
                    np.random.rand(len(population[i])) < cr_pop[i], mutant,
                    population[i]
                )
            ...
            # Local Search
            for i in range(self._n_population):
                local_candidate = population[i] + \
                    np.random.uniform(-0.1, 0.1, len(population[i]))
